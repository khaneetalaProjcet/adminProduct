<template>
    <div>
                ...
                <v-card-text>
                    <v-tabs-window v-model="tab">
                        <v-tabs-window-item value="one">
                            ...
                            <v-card title="نامشخص">
                                <v-data-table :headers="PendingCartToCartHeader" :items="PendingCartToCartData"
                                    :loading="PendingCartToCartLoading">
                                    ...
                                    <template v-slot:item.action="{ item }">
                                        <v-icon class="me-2" size="small" icon="ri-refund-2-line" color="#d4af37"
                                            @click="PendingCartToCartInfo(item)"></v-icon>
                                    </template>
                                </v-data-table>
                            </v-card>
                        </v-tabs-window-item>
                        <v-tabs-window-item value="two">
                            ...
                            <v-card title="موفق">
                                <v-data-table :headers="CompleteCartToCartHeader" :items="CompleteCartToCartData"
                                    :loading="CompleteCartToCartLoading">
                                    <template v-slot:item.totalPrice="{ item }">
                                        <p>{{ formatNumber(item.totalPrice) }}</p>
                                    </template>
                                    <template v-slot:item.status="{ item }">
                        ...
                        <v-tabs-window-item value="three">
                            ...
                            <v-card title="ناموفق">
                                <v-data-table :headers="FailedCartToCartHeader" :items="FailedCartToCartData"
                                    :loading="CompleteCartToCartLoading">
                                    <template v-slot:item.totalPrice="{ item }">
                                        <p>{{ formatNumber(item.totalPrice) }}</p>
                                    </template>
        ...
        <!-- CartToCart Dialog -->
        <v-dialog v-model="CartToCartDialog" max-width="600" class="dialog">
            <v-card class="dialog-card">
                ...
                <div class="d-flex flex-column flex-md-row justify-space-between w-100 px-4 py-8 user-dialog-info">
                    <div class="d-flex flex-column w-100 px-4">
                        <div class="d-flex align-items-center my-2">
                            <p>نام: </p>
                            <p class="mx-2">{{ CartToCartDetail.buyer.firstName }}</p>
                        </div>
                        <div class="d-flex align-items-center my-2 user-price">
                            <p>کد ملی: </p>
                            <p class="mx-2">{{ CartToCartDetail.buyer.nationalCode }}</p>
                        </div>
                        <div class="d-flex align-items-center my-2">
                            <p>مبلغ کل: </p>
                            <p class="mx-2">{{ formatNumber(CartToCartDetail.totalPrice) }} تومان</p>
                        </div>
                    </div>
                    <div class="d-flex flex-column w-100 px-4">
                        <div class="d-flex align-items-center my-2">
                            <p>نام خانوادگی: </p>
                            <p class="mx-2">{{ CartToCartDetail.buyer.lastName }}</p>
                        </div>

                        <div class="d-flex align-items-center my-2 user-price">
                            <p>قیمت لحظه ای طلا: </p>
                            <p class="mx-2">{{ formatNumber(CartToCartDetail.goldPrice) }} تومان</p>
                        </div>
                        <div class="d-flex align-items-center my-2 user-price">
                            <p>وزن طلای خریداری شده: </p>
                            <p class="mx-2">{{ formatNumber(CartToCartDetail.goldWeight) }} تومان</p>
                        </div>
                    </div>
                </div>
                <div class="form-box">
                    <v-form ref="form" @submit.prevent="submitCartToCart">
                        <v-btn type="submit" size="large" class="my-2" :loading="CartToCartSubmitLoading" block>بررسی
                            مجدد
                            پرداخت</v-btn>
                    </v-form>
                </div>
                <v-card-actions>
                    ...
                    <v-btn text="بستن" @click="CartToCartDialog = false" size="large" class="m-3"></v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
const PendingCartToCartLoading = ref(false);
const CartToCartSubmitLoading = ref(false);
const tab = ref(null);
const PendingCartToCartHeader = ref([
    {
        title: 'نام',
        key: 'buyer.firstName',
    },
...
const PendingCartToCartData = ref();
const CompleteCartToCartHeader = ref([
    {
        title: 'نام',
        key: 'buyer.firstName',
    },
...
const FailedCartToCartHeader = ref([
    {
        title: 'نام',
        key: 'buyer.firstName',
    },
...
const CompleteCartToCartData = ref();
const CompleteCartToCartLoading = ref();
const FailedCartToCartLoading = ref();
const FailedCartToCartData = ref();
const InitCartToCartData = ref();
const InitCartToCartLoading = ref();
const CartToCartDetail = ref();
const CartToCartDialog = ref(false);
const CartToCartSubmitDetail = ref({
    authority: '',
    id: '',
})
...
const GetPendingCartToCartList = async () => {
    try {
        PendingCartToCartLoading.value = true;
        const response = await GoldBoxService.CartToCartList('waitForTransport');
        PendingCartToCartData.value = response;
        return response
    } catch (error) {
        ...
        PendingCartToCartLoading.value = false;
    }
};

const GetCompleteCartToCartList = async () => {
    try {
        CompleteCartToCartLoading.value = true;
        const response = await GoldBoxService.CartToCartList('completedAfterTransport');
        CompleteCartToCartData.value = response;
        return response
    } catch (error) {
        if (error.response.status == 401) {
            localStorage.clear();
        ...
        CompleteCartToCartLoading.value = false;
    }
};

const GetFailedCartToCartList = async () => {
    try {
        FailedCartToCartLoading.value = true;
        const response = await GoldBoxService.CartToCartList('failedAfterTransport');
        FailedCartToCartData.value = response;
        return response
    } catch (error) {
        if (error.response.status == 401) {
            localStorage.clear();
        ...
        FailedCartToCartLoading.value = false;
    }
};
...
const PendingCartToCartInfo = (item) => {
    CartToCartDialog.value = true;
    CartToCartDetail.value = item;
    CartToCartSubmitDetail.value.id = item.id;
    CartToCartSubmitDetail.value.authority = item.authority;
}

const submitCartToCart = async () => {
    try {
        CartToCartSubmitLoading.value = true;
        const response = await GoldBoxService.SubmitCartToCart(CartToCartSubmitDetail.value);
        successMsg.value = response.msg;
        alertSuccess.value = true;
        setTimeout(() => {
            alertSuccess.value = false;
        }, 5000)
        GetPendingCartToCartList();
        GetCompleteCartToCartList();
        GetFailedCartToCartList();
        CartToCartDialog.value = false;
        return response
    } catch (error) {
        if (error.response.status == 401) {
            localStorage.clear();
        ...
        CartToCartSubmitLoading.value = false;
    }
}
...
const SubmitFilter = async (status) => {
    try {
        ...
        CompleteCartToCartLoading.value = false;
        FailedCartToCartLoading.value = false;
        InitCartToCartLoading.value = false;
    }
}
