<template>
    <div>
            ...
            <v-col cols="12">
                <v-card-text>
                    ...
                    <v-card title="شارژ کیف پول">
                        <ul class="listGuide ">
                            <li>
                                <span>حسابداری شارژ کیف پول نشان دهنده تراکنش افرادی هست که توسط ادمین، کیف پول آنها
                                    شارژ شده
                                    است.
                                </span>
                            </li>
                        ...
                        <v-data-table :headers="chargeCardHeader" :items="chargeCardData" :loading="chargeCardLoading">
                            <template v-slot:item.amount="{ item }">
                                <p>{{ formatNumber(item.amount) }}</p>
                            </template>
                            <template v-slot:item.status="{ item }">
                                <div>
                                    <v-chip :text="item.status == 'completed' ? 'تایید شده' : 'در انتظار تایید'"
        ...
        <!-- chargeWallet Dialog -->
        <v-dialog v-model="chargeWalletDialog" max-width="700" class="dialog">
            <v-card class="dialog-card">
                ...
                <v-row class="my-3 px-3">
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>نام: </p>
                            <p class="mx-2">{{ chargeWalletDetail?.user?.firstName }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>نام خانوادگی: </p>
                            <p class="mx-2">{{ chargeWalletDetail?.user?.lastName }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>کد ملی: </p>
                            <p class="mx-2">{{ chargeWalletDetail?.user?.nationalCode }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>شماره موبایل: </p>
                            <p class="mx-2">{{ chargeWalletDetail?.user?.phoneNumber }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>مبلغ: </p>
                            <p class="mx-2">{{ formatNumber(chargeWalletDetail?.amount) }} تومان</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>تاریخ : </p>
                            <p class="mx-2">{{ chargeWalletDetail?.date }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>زمان : </p>
                            <p class="mx-2">{{ chargeWalletDetail?.time }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>شماره پیگیری :</p>
                            <p class="mx-2">{{ chargeWalletDetail?.invoiceId }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>موجودی کیف پول :</p>
                            <p class="mx-2">{{ formatNumber(chargeWalletDetail?.user?.wallet?.balance) }} تومان</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>کارشناس ثبت کننده : </p>
                            <p class="mx-2">{{ chargeWalletDetail?.admin }}</p>
                        </div>
                    </v-col>
                    <v-col cols="6" class="my-1">
                        <div class="d-flex align-items-center">
                    ...
                    <v-col cols="12" class="my-1">
                        <div class="d-flex align-items-center">
                            <p>شماره شبا : </p>
                            <p class="mx-2">{{ chargeWalletDetail?.shebaNumber }}</p>
                        </div>
                    </v-col>
                </v-row>
                <div class="form-box">
                    <v-form ref="form" @submit.prevent="">
                        <v-textarea v-model="chargeWalletDetail.description" label="توضیحات" variant="outlined"
                            rows="2"></v-textarea>
                        <div class="d-flex justify-space-between">
                            <v-btn type="submit" size="large" class="my-2" color="#388E3C"
                                :loading="chargeWalletSubmitLoading" @click="submitchargeWallet(1)">تایید واریز
                            </v-btn>
                            <v-btn type="submit" size="large" class="my-2" color="error"
                                :loading="chargeWalletRejectLoading" @click="submitchargeWallet(2)">رد واریز
                            </v-btn>
                        </div>
                    </v-form>
                </div>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text="بستن" @click="chargeWalletDialog = false" size="large" class="m-3"></v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
const chargeCardLoading = ref(false);
const chargeWalletRejectLoading = ref(false);
const chargeWalletSubmitLoading = ref(false);
const chargeCardHeader = ref([
    {
        title: 'نام',
        key: 'user.firstName',
    },
...
const chargeCardData = ref();
const chargeWalletDetail = ref({
    description: '',
    id: '',
})
const chargeWalletDialog = ref(false);


const GetChargeWalletList = async () => {
    try {
        chargeCardLoading.value = true;
        const response = await WalletService.chargeList();
        chargeCardData.value = response.data;
        return response
    } catch (error) {
        ...
        chargeCardLoading.value = false;
    }
};

const DepositInfo = (item) => {
    chargeWalletDialog.value = true;
    chargeWalletDetail.value = item;
    chargeWalletDetail.value.id = item.id;
};

const submitchargeWallet = async (status) => {
    try {
        if (status == 1) {
            chargeWalletSubmitLoading.value = true;
        } else {
            chargeWalletRejectLoading.value = true;
        }
        const response = await WalletService.submitAccountingCharge(chargeWalletDetail.value, status);
        GetChargeWalletList();
        chargeWalletDialog.value = false;
        return response
    } catch (error) {
        if (error.response.status == 401) {
            localStorage.clear();
        ...
        chargeWalletSubmitLoading.value = false;
        chargeWalletRejectLoading.value = false;
    }
}
